...
for episode in range(0, 500):
    ...
    for step in range(0, 1000):
        ...
        if step % 100 == 0:
            print(f"Episode {episode}, Step {step}, Total Reward: {total_reward:.2f}")
    ...
    print(f"Episode {episode}/500, Reward: {total_reward}")

    if episode % 100 == 0:
        torch.save(q_network.state_dict(), f"improved_DQN_500_ep{episode}.pt")
    
torch.save(q_network.state_dict(), 'improved_dqn_500.pt')
...
plt.legend()
plt.grid(True)
plt.savefig("improved_dqn_500_rewards.png")
plt.show()
